...
export default function CardSliderProgram() {
  ...
  const closeModal = () => {
    ...
    animate(
      ...
      {
        duration: 0.2,
        onComplete: () => setSelectedProgram(null),
      }
    )
  }
        ...
        <CarouselContent>
          {programs.map((program) => (
            <CarouselItem key={program.id} className="basis-full md:basis-1/2 lg:basis-1/3">
              <div className="rounded-xl border-2 border-yellow-200 border-dotted overflow-hidden bg-white py-6 hover:shadow-lg transition-shadow">
                <div className="flex justify-center">
                  <div className="bg-yellow-50 p-6 w-fit rounded-full hover:scale-110 transition-transform">
                    <Image src={program.image} alt={program.title} width={75} height={75} />
                  </div>
                </div>
                ...
                <div className="px-6 mt-6 flex flex-col flex-1">
                  ...
                  <div className="flex justify-center mt-6">
                    <button
                      ...
                      className="border border-yellow-200 py-2 px-4 rounded-lg text-yellow-600 font-medium hover:bg-yellow-50 transition"
                    >
                      Lihat Detail
                    </button>
                  </div>
      ...
      {selectedProgram && (
        ...
        >
          ...
          >
            ...
            <h2 className="text-2xl font-bold text-gray-800 mb-4">{selectedProgram.title}</h2>

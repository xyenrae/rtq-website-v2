...
import { deleteKategori, type BeritaKategori } from '@/lib/berita-kategori'
import { getCategoryStyle } from '@/lib/berita-kategori' // Pastikan helper ini di-export dari lib/berita

// ─── Props Interface (Exported) ──────────────────────────────────────────────

export interface ModalDeleteKategoriProps {
  open: boolean
  onClose: () => void
...
export function ModalDeleteKategori({
  open,
  onClose,
  kategori,
  beritaCount,
  ...
  const style = getCategoryStyle(kategori.nama)
        ...
        >
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-destructive/5 border border-destructive/20">
              ...
              <p className="text-xs text-destructive leading-relaxed">
                ...
                {beritaCount > 0 && (
                  <>
                    {' '}
                    Terdapat <span className="font-semibold">{beritaCount} berita</span> yang
            ...
            <div className="rounded-xl border border-border bg-muted/30 p-4 flex items-center gap-4">
              ...
              >
                <IconTag size={18} className={style.text} />
              </div>
              <div className="min-w-0">
                <p className="font-bold text-foreground text-sm">{kategori.nama}</p>
                <p className="text-xs text-muted-foreground mt-0.5">{beritaCount} berita terkait</p>
              </div>
              <span
                className={cn(

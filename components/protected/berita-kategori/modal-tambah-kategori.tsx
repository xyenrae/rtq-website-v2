...
import { IconPlus, IconX, IconCheck, IconLoader2, IconFileDescription } from '@tabler/icons-react'
import * as DialogPrimitive from '@radix-ui/react-dialog'
import { type BeritaKategori } from '@/lib/berita'
import { insertKategori, getCategoryStyle } from '@/lib/berita-kategori'
...
export interface ModalTambahKategoriProps {
  open: boolean
  onClose: () => void
  onSave: (kategori: BeritaKategori) => void
}

// ─── Component (Exported) ────────────────────────────────────────────────────

export function ModalTambahKategori({ open, onClose, onSave }: ModalTambahKategoriProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      const result = await insertKategori({
        nama: nama.trim(),
      ...
      setError({ nama: msg })
      toast.error('Gagal menambahkan kategori', { description: msg, duration: 5000 })
    } finally {
      setSaving(false)
    }
  }
          ...
          <div className="flex items-start justify-between px-6 pt-5 pb-4 border-b border-border">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Buat kategori berita baru</p>
              </div>
            </div>
          ...
          <div className="px-6 py-5 space-y-5">
            ...
            <div>
              ...
              <input
                id="tambah-nama"
                maxLength={50}
                placeholder="Contoh: Pendidikan, Kegiatan, Pengumuman..."
                value={nama}
                onChange={(e) => {
            ...
            {nama.trim() && (
              <div className="rounded-xl border border-border bg-muted/30 p-4 space-y-3">
                ...
                <p className="text-[10px] text-muted-foreground pt-1 border-t border-border/50">
                  Begini tampilan kategori ini di tabel berita.
                </p>
              </div>
            )}
          </div>
